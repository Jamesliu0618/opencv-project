...
namespace PCBInspection.Tests.Unit
{
    ...
    {
        [Test]
        public void DetectRotatedComponent_WithMoments_ReturnsCentroidCloseToTrue()
        {
            ...
            var pts = box.Points();
            var ptsInt = pts.Select(p => new Point((int)System.Math.Round(p.X), (int)System.Math.Round(p.Y))).ToArray();
            ...
            var c = comps[0];
            Assert.AreEqual(200, (int)System.Math.Round(c.CenterX_Px));
            Assert.AreEqual(150, (int)System.Math.Round(c.CenterY_Px));
        }

        [Test]
        public void DetectComponents_WithNoise_ClosingRecoversComponents()
        {
            var img = new Mat(new Size(400, 300), MatType.CV_8UC3, Scalar.White);
