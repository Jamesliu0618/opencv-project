...
namespace PCBInspection.Tests.Unit
{
	...
	{
		[Test] public void DetectComponents_TwoRectangles_ReturnsTwoComponentsWithCorrectPositions()
		{
			...
			var c0 = comps[0];
			var c1 = comps[1];
			Assert.AreEqual(100, (int)Math.Round(c0.CenterX_Px));
			Assert.AreEqual(100, (int)Math.Round(c0.CenterY_Px));
			Assert.AreEqual(220, (int)Math.Round(c1.CenterX_Px));
			Assert.AreEqual(100, (int)Math.Round(c1.CenterY_Px));

			// mm coordinates
			(double x0mm, double y0mm) = Calibration.PixelToMm(c0.CenterX_Px, c0.CenterY_Px);
