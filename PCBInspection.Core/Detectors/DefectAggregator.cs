    ...
    public class DefectAggregator
    {
        ...
        public AggregatedResult Detect(Mat image, List<Component> components = null)
        {
            ...
            foreach (var detector in _detectors)
            {
                ...
                {
                    // 記錄錯誤但繼續執行其他偵測器
                    System.Diagnostics.Debug.WriteLine($"Detector {detector.Name} failed: {ex.Message}");
                }
            }
        ...
        {
            if (defects.Count <= 1) return defects;
        ...
        private InspectionDecision DetermineDecision(int maxSeverity)
        {
            ...
            // 3: 人工覆核
            // 4-5: 自動 NG
            if (maxSeverity >= 4) return InspectionDecision.NG;
            if (maxSeverity == 3) return InspectionDecision.REVIEW;
