namespace PCBInspection.Core.Services
{
    ...
    public static class AnnotationService
    {
        ...
        /// 繪製完整標註影像
        /// </summary>
        public static Mat Annotate(Mat image, List<Component> components, List<Defect> defects, 
            InspectionDecision decision, AnnotationOptions options = null)
        {
        ...
        private static void DrawComponent(Mat image, Component comp, AnnotationOptions opts)
        {
            ...
            if (opts.ShowMeasurements && comp.WidthMm > 0)
            {
                ...
                Cv2.PutText(image, label, labelPos, HersheyFonts.HersheySimplex, 
                    opts.FontScale, opts.MeasurementColor, 1);
            }
        ...
        private static void DrawDefect(Mat image, Defect defect, AnnotationOptions opts)
        {
            ...
            var rect = new Rect(defect.BoundingBox[0], defect.BoundingBox[1], 
                                defect.BoundingBox[2], defect.BoundingBox[3]);

            // 根據嚴重度選擇顏色
            var color = GetSeverityColor(defect.Severity);

            // 繪製邊框（嚴重度越高線條越粗）
            var thickness = Math.Min(defect.Severity, 4);
            Cv2.Rectangle(image, rect, color, thickness);

            // 繪製瑕疵類型標籤
            var label = $"{defect.Type} (S{defect.Severity})";
            var labelPos = new Point(rect.X, rect.Y - 8);
            Cv2.PutText(image, label, labelPos, HersheyFonts.HersheySimplex, 
                opts.FontScale * 0.8, color, 1);

            // 嚴重瑕疵加上警告圖示
            if (defect.Severity >= 4)
            {
                var iconPos = new Point(rect.X + rect.Width + 5, rect.Y + rect.Height / 2);
        ...
        private static void DrawDecision(Mat image, InspectionDecision decision, AnnotationOptions opts)
        {
            ...
            var color = decision == InspectionDecision.OK ? new Scalar(0, 200, 0) :
                       decision == InspectionDecision.REVIEW ? new Scalar(0, 200, 200) :
                       new Scalar(0, 0, 255);

            // 繪製半透明背景
            var bgRect = new Rect(10, 10, 150, 50);
            ...
            // 繪製判定文字
            Cv2.PutText(image, text, new Point(25, 45), HersheyFonts.HersheySimplex, 
                1.2, Scalar.White, 2);
        }
        ...
        private static void DrawTimestamp(Mat image, AnnotationOptions opts)
        {
            ...
            var pos = new Point(image.Width - 200, image.Height - 15);
            Cv2.PutText(image, ts, pos, HersheyFonts.HersheySimplex, 0.5, new Scalar(200, 200, 200), 1);
        }
        ...
        private static Scalar GetSeverityColor(int severity)
        {
            switch (severity)
            {
                case 1: return new Scalar(200, 200, 0);   // 藍綠色 - 輕微
                case 2: return new Scalar(0, 200, 200);   // 黃色 - 輕度
                case 3: return new Scalar(0, 165, 255);   // 橙色 - 中度
                case 4: return new Scalar(0, 0, 255);     // 紅色 - 嚴重
                case 5: return new Scalar(0, 0, 180);     // 深紅色 - 致命
        ...
        public static string SaveAnnotatedImage(Mat annotatedImage, string outputDir, string pcbId)
        {
            ...
            var filename = $"{pcbId}_annotated_{DateTime.UtcNow:yyyyMMddHHmmss}.png";
