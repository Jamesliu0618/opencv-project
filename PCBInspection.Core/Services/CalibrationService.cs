...
namespace PCBInspection.Core.Services
{
    ...
    public static class CalibrationService
    {
        ...
        {
            if (!File.Exists(path)) throw new FileNotFoundException("Calibration file not found", path);
            var txt = File.ReadAllText(path);
            var meta = JsonConvert.DeserializeObject<CalibrationMetadata>(txt);
            if (meta == null) throw new InvalidDataException("Invalid calibration file content");
